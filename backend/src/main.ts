...
app.get('/metrics', (req: Request, res: Response) => {
  ...
  const frozen = vendorName.toLowerCase().includes('vardhaman');

  let metricsData;
  if (vendorName.includes('Matrix')) {
    metricsData = {
      nodes: "8,912",
      alerts: 412,
      shield: "41.2%",
  ...
  } else if (vendorName.includes('Apex')) {
    metricsData = {
      nodes: "14,102",
      alerts: 14,
      shield: "99.1%",
  ...
  } else if (vendorName.includes('SecureLogix')) {
    metricsData = {
      nodes: "15000",
      alerts: 5,
      shield: "99.5%",
  ...
  try {
    const logEntry = `[${new Date().toISOString()}] Vendor: ${vendorName} | Metrics: ${JSON.stringify(metricsData)} | Frozen: ${frozen}\n`;
    fs.appendFileSync('security-audit.log', logEntry);
  } catch (err) {
...
function getTxHash(): string {
  return '0x' + randomBytes(16).toString('hex').toUpperCase();
}

/** Push a log entry with cryptographic trace hash */
function pushLog(agentId: string, action: string, result: string) {
...
function dispatchAlert(type: 'INFO' | 'ALERT' | 'CRITICAL', source: string, message: string) {
...
setInterval(() => {
  ...
  if (taskQueue.length > 0) {
    ...
    if (idleAgent) {
      ...
      setTimeout(() => {
        const isFraud =
          nextTask.incorporationNumber.toUpperCase().includes('FRAUD') ||
...
const agentWatchdogMap = new Map<string, number>();
setInterval(() => {
  const now = Date.now();
  agents.forEach(agent => {
...
app.get('/api/v1/swarm/alerts', (_req: Request, res: Response) => {
  res.json(alerts);
});

/** GET /api/v1/swarm/health – System health telemetry */
app.get('/api/v1/swarm/health', (_req: Request, res: Response) => {
...
app.post('/api/v1/organization/register', (req: Request, res: Response) => {
  ...
  const isDuplicate = organizations.some(
    o => o.incorporationNumber.toLowerCase() === incorporationNumber.toLowerCase()
  );
  if (isDuplicate) {
  ...
  const newOrg: Organization = { id: Date.now(), name, incorporationNumber, retryCount: 0 };
...
app.post('/api/v1/agent/spawn', (req: Request, res: Response) => {
  ...
  const newAgent: Agent = { name, role, status: 'IDLE', busySince: null, currentTask: null };
...
app.get('/api/v1/reports/compile', (_req: Request, res: Response) => {
  const highRisk = organizations.filter(o =>
    o.incorporationNumber.toUpperCase().includes('RISK') ||
    o.incorporationNumber.toUpperCase().includes('FRAUD')
  ).length + fraudInterceptionsCount;

  // Cryptographic signature
  const sigData = `${organizations.length}-${agents.length}-${highRisk}-${Date.now()}`;
  const sigHash = createHash('sha256').update(sigData).digest('hex').substring(0, 64);

  // Remediation recommendations for suspicious orgs
  const suspiciousOrgs = organizations.filter(o =>
    o.incorporationNumber.toUpperCase().includes('RISK') ||
    o.incorporationNumber.toUpperCase().includes('FRAUD')
  );
